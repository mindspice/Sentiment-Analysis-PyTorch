...
class SentimentClassifierWithCNN(nn.Module):
    def __init__(self, input_dim, embedding_dim, hidden_dim, output_dim=1, num_layers=1, dropout=0.0,
        ...
        super().__init__()
        self.embedding = nn.Embedding(input_dim, embedding_dim)
        self.conv1d = nn.Conv1d(embedding_dim, embedding_dim, kernel_size=3, padding=1)
        self.rnn = nn.LSTM(embedding_dim, hidden_dim, num_layers=num_layers, batch_first=True, dropout=dropout,
                           bidirectional=bidirectional)
        self.fc = nn.Linear(hidden_dim * 2 if bidirectional else hidden_dim, output_dim)
...
class SentimentClassifierWithDualCNN(nn.Module):
    def __init__(self, input_dim, embedding_dim, hidden_dim, output_dim=1, num_layers=1, dropout=0.0,
                 bidirectional=True):
        super().__init__()
        self.embedding = nn.Embedding(input_dim, embedding_dim)
        self.conv1d_1 = nn.Conv1d(embedding_dim, embedding_dim, kernel_size=3, padding=1)
        self.conv1d_2 = nn.Conv1d(embedding_dim, embedding_dim, kernel_size=5, padding=2)
    ...
    def forward(self, text):
        embedded = self.embedding(text)
        embedded = embedded.permute(0, 2, 1)
        conv_out = self.conv1d_1(embedded)
        conv_out = F.relu(conv_out)
        conv_out = self.dropout(conv_out)
        conv_out = self.conv1d_2(conv_out)
        conv_out = F.relu(conv_out)
        conv_out = self.dropout(conv_out)
        conv_out = conv_out.permute(0, 2, 1)
...
def get_cnn_lstm_model(vocab_size):
    ...
    dropout_rate = 0.25
    bidirectional = True
    return SentimentClassifierWithDualCNN(input_dim, embedding_dim, hidden_dim, output_dim, num_layers, dropout_rate,
                               bidirectional)
